...
public isolated client class ServerReflectionClient {
    *AbstractClientEndpoint;
    ...
    isolated remote function ServerReflectionInfo() returns ServerReflectionInfoStreamingClient|Error {
        StreamingClient sClient = check self.grpcClient->executeBidirectionalStreaming("grpc.reflection.v1alpha.ServerReflection/ServerReflectionInfo");
...
public client class ServerReflectionInfoStreamingClient {
    ...
    isolated remote function sendServerReflectionRequest(ServerReflectionRequest message) returns Error? {
        return self.sClient->send(message);
    }

    isolated remote function sendContextServerReflectionRequest(ContextServerReflectionRequest message) returns Error? {
        return self.sClient->send(message);
    }

    isolated remote function receiveServerReflectionResponse() returns ServerReflectionResponse|Error? {
    ...
    isolated remote function receiveContextServerReflectionResponse() returns ContextServerReflectionResponse|Error? {
        var response = check self.sClient->receive();
        if response is () {
            return response;
        } else {
...
public client class ServerReflectionServerReflectionResponseCaller {
    ...
    isolated remote function sendContextServerReflectionResponse(ContextServerReflectionResponse response) returns Error? {
        return self.caller->send(response);
    }
