...
@test:Config {enable: true}
function testHello55ReturnWithCaller() returns error? {
    JwtIssuerConfig config = {
        username: "admin",
        issuer: "wso2",
        audience: ["ballerina"],
    ...
    helloWorld55Client hClient = check new ("http://localhost:9155");
    string|Error result = hClient->hello55UnaryWithReturn(ctxString);
    if result is Error {
        test:assertFail(result.message());
    } else {
        test:assertEquals(result, "Hello from client");
...
@test:Config {enable: true}
function testHello55UnaryWithReturnUnauthenticated() returns error? {
    map<string|string[]> requestHeaders = {
        "authorization": "bearer asfasfasf"
    };
    ContextString ctxString = {
...
@test:Config {enable: true}
function testHello55UnaryWithReturnInvalidPermission() returns error? {
    JwtIssuerConfig config = {
        username: "admin",
        issuer: "wso2",
        audience: ["ballerina"],
