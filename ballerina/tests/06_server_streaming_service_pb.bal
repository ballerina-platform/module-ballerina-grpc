...
public isolated client class HelloWorld45Client {
    *AbstractClientEndpoint;
    ...
    public isolated function init(string url, *ClientConfiguration config) returns Error? {
        ...
        check self.grpcClient.initStub(self, ROOT_DESCRIPTOR_06_SERVER_STREAMING_SERVICE, getDescriptorMap06ServerStreamingService());
    }

    isolated remote function lotsOfReplies(string|wrappers:ContextString req) returns stream<string, error?>|Error {
        ...
        var payload = check self.grpcClient->executeServerStreaming("grpcservices.HelloWorld45/lotsOfReplies", message, headers);
    ...
    isolated remote function lotsOfRepliesContext(string|wrappers:ContextString req) returns wrappers:ContextStringStream|Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
