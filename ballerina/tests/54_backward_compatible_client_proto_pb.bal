...
public isolated client class ProductCatalogClient {
    *AbstractClientEndpoint;
    ...
    isolated remote function getProduct(string|wrappers:ContextString req) returns Product|Error {
    ...
    isolated remote function getProductContext(string|wrappers:ContextString req) returns ContextProduct|Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
    ...
    isolated remote function sendCustomerContext(ChangedCustomer|ContextChangedCustomer req) returns wrappers:ContextString|Error {
        map<string|string[]> headers = {};
        ChangedCustomer message;
        if req is ContextChangedCustomer {
            message = req.content;
...
public client class ProductCatalogProductCaller {
    ...
    isolated remote function sendProduct(Product response) returns Error? {
        return self.caller->send(response);
    }

    isolated remote function sendContextProduct(ContextProduct response) returns Error? {
        return self.caller->send(response);
    }
