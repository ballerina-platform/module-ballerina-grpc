...
service "StreamingCalcService" on ep {
    ...
    remote function incrementalSum1(StreamingCalcServiceIntCaller caller, stream<int, grpc:Error?> clientStream) returns error|() {
       int sum = 0;
       error? e = clientStream.forEach(function (int value) {
           sum += value;
           checkpanic caller->sendInt(sum);
    ...
    remote function incrementalSum2(StreamingCalcServiceIntCaller caller, stream<int, grpc:Error?> clientStream) returns ()|error {
       int sum = 0;
       error? e = clientStream.forEach(function (int value) {
           sum += value;
           checkpanic caller->sendInt(sum);
