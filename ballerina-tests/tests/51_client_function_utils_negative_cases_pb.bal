...
public isolated client class HelloWorld51Client {
    *grpc:AbstractClientEndpoint;
    ...
    public isolated function init(string url, *grpc:ClientConfiguration config) returns grpc:Error? {
        ...
        check self.grpcClient.initStub(self, CLIENT_FUNCTION_UTILS_NEGATIVE_CASES_DESC);
    }

    isolated remote function stringUnary(string|wrappers:ContextString req) returns string|grpc:Error {
    ...
    isolated remote function stringUnaryContext(string|wrappers:ContextString req) returns wrappers:ContextString|grpc:Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
    ...
    isolated remote function stringServerStreaming(string|wrappers:ContextString req) returns stream<string, grpc:Error?>|grpc:Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
    ...
    isolated remote function stringServerStreamingContext(string|wrappers:ContextString req) returns wrappers:ContextStringStream|grpc:Error {
        map<string|string[]> headers = {};
        string message;
        if req is wrappers:ContextString {
            message = req.content;
...
public isolated client class StringClientStreamingStreamingClient {
...
public isolated client class StringBiDiStreamingClient {
    private final grpc:StreamingClient sClient;
...
public isolated client class HelloWorld51StringCaller {
