...
@test:Config {enable:true}
function testClientHelloWorldTimestamp() {
    var result = assertGeneratedSources("client", "helloWorldTimestamp.proto", "helloWorldTimestamp_pb.bal", "helloWorld_sample_service.bal", "helloWorld_sample_client.bal", "tool_test_client_8");
    if (result is error) {
        test:assertFail("Failed to assert generated sources");
    }
}
