...
public client class HelloWorld1BlockingClient {
    ...
    *AbstractClientEndpoint;
    ...
    map<string[]>]|Error) {
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testInputNestedStruct", req, headers);
        anydata result = ();
    ...
    map<string[]>]|Error) {
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testOutputNestedStruct", req, headers);
        anydata result = ();
        map<string[]> resHeaders;
        [result, resHeaders] = payload;
    ...
    ([StockQuote, map<string[]>]|Error) {
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testInputStructOutputStruct", req, headers);
        anydata result = ();
        map<string[]> resHeaders;
        [result, resHeaders] = payload;
    ...
    |Error) {
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testInputStructNoOutput", req, headers);
        anydata result = ();
        map<string[]> resHeaders;
    ...
    map<string[]>]|Error) {
        Empty req = {};
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testNoInputOutputStruct", req, headers);
        anydata result = ();
        map<string[]> resHeaders;
        [result, resHeaders] = payload;
    ...
    map<string[]>]|Error) {
        Empty req = {};
        [anydata, map<string[]>] payload = check self.grpcClient->blockingExecute("grpcservices.HelloWorld/testNoInputOutputArray", req, headers);
        anydata result = ();
        map<string[]> resHeaders;
        [result, resHeaders] = payload;
...
public client class HelloWorldClient {
    ...
    *AbstractClientEndpoint;
    ...
    isolated remote function testInputNestedStruct(Person req, service object {} msgListener, map<string[]> headers = {})
    returns (Error?) {
        return self.grpcClient->nonBlockingExecute("grpcservices.HelloWorld/testInputNestedStruct", req, msgListener,
         headers);
    }

    isolated remote function testOutputNestedStruct(string req, service object {} msgListener, map<string[]> headers = {})
    returns (Error?) {
        return self.grpcClient->nonBlockingExecute("grpcservices.HelloWorld/testOutputNestedStruct", req, msgListener,
            headers);
    }

    isolated remote function testInputStructOutputStruct(StockRequest req, service object {} msgListener, map<string[]>
    ...
    isolated remote function testInputStructNoOutput(StockQuote req, service object {} msgListener, map<string[]> headers =
    {}) returns (Error?) {
        return self.grpcClient->nonBlockingExecute("grpcservices.HelloWorld/testInputStructNoOutput", req, msgListener,
            headers);
    }

    isolated remote function testNoInputOutputStruct(Empty req, service object {} msgListener, map<string[]> headers = {})
    returns (Error?) {
        return self.grpcClient->nonBlockingExecute("grpcservices.HelloWorld/testNoInputOutputStruct", req, msgListener,
            headers);
    }

    isolated remote function testNoInputOutputArray(Empty req, service object {} msgListener, map<string[]> headers = {})
    returns (Error?) {
