...
HelloWorld5BlockingClient helloWorldBlockingEp = new ("http://localhost:9095");
...
function testInvalidRemoteMethod() {
    string name = "WSO2";
    [string, Headers]|Error unionResp = helloWorldBlockingEp->hello(name);
...
function testInvalidInputParameter() {
    int age = 10;
    [int, Headers]|Error unionResp = helloWorldBlockingEp->testInt(age);
    if (unionResp is Error) {
...
function testInvalidOutputResponse() {
    float salary = 1000.5;
    [float, Headers]|Error unionResp = helloWorldBlockingEp->testFloat(salary);
    if (unionResp is Error) {
